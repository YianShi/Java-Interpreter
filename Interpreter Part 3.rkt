...
(define interpret-statement
  (lambda (statement environment return break continue throw)
    (cond
      ...
      ((eq? 'return-not-in-main (statement-type statement)) (return environment))
      ...
      ((eq? 'function (car statement)) (if (eq? 'main (cadr statement))
                                           (interpret-main statement environment return break continue throw)
                                           (interpret-function statement environment)))
      ((eq? 'funcall (car statement)) (interpret-funcall statement (push-frame environment) return break continue throw))
      (else (eval-expression statement environment throw)))))
...
(define interpret-assign
  (lambda (statement environment throw)
    (if (and (list? (get-assign-rhs statement)) (eq? 'funcall (car (get-assign-rhs statement))))
        (update (get-assign-lhs statement) (eval-expression (get-assign-rhs statement) environment throw) (interpret-funcall-ignore-return (get-assign-rhs statement)
                                                                                                                                           environment
                                                                                                                                           (lambda (v) v)
...
; add a defined function to the environment
...
(define interpret-main
  (lambda (statement environment return break continue throw)
    (interpret-statement-list (get-function-body statement) (push-frame environment) return break continue throw)))

; call function. get function environment first. hide the return
(define interpret-funcall
  (lambda (statement environment return break continue throw)
    (call/cc
     (lambda (return)
    (let ((s (interpret-statement-list (mark-return (cadr (lookup (get-function-name statement) environment)))
                                       (get-function-environment (get-global-variable (get-function-name statement) environment) (eval-list (get-function-parameter statement) environment throw) (get-function-variable (get-function-name statement) environment))
                                       return
                                       break
                                       continue
                                       throw)))
    (if (list? s) (get-new-environment (get-function-name statement) environment s) s))))))

; calcualte function value with return
(define call-function-with-value
  (lambda (statement environment return break continue throw)
    (call/cc
     (lambda (return)
       (interpret-statement-list (cadr (lookup (get-function-name statement) environment))
                                 (get-function-environment (get-global-variable (get-function-name statement) environment) (eval-list (get-function-parameter statement) environment throw) (get-function-variable (get-function-name statement) environment))
                                 return
                                 break
                                 continue
...
; used to handle side-effect. ignore return.
(define interpret-funcall-ignore-return
  (lambda (statement environment return break continue throw)
    (call/cc
     (lambda (return)
    (let ((s (interpret-statement-list (ignore-return (cadr (lookup (get-function-name statement) environment)))
                                       (get-function-environment (get-global-variable (get-function-name statement) environment) (eval-list (get-function-parameter statement) environment throw) (get-function-variable (get-function-name statement) environment))
                                       return
                                       break
                                       continue
...
(define eval-expression
  (lambda (expr environment throw)
    (cond
      ...
      ((eq? (car expr) 'funcall) (call-function-with-value expr environment (lambda (v) v)
                                  (lambda (env) (myerror "Break used outside of loop")) (lambda (env) (myerror "Continue used outside of loop"))
                                   throw))
...
; change 'return to 'return-not-in-main
(define mark-return
  (lambda (body)
    (cond
      ((null? body) '())
      ((list? (car body)) (cons (mark-return (car body)) (mark-return (cdr body))))
      ((eq? (car body) 'function) body)
      ((eq? (car body) 'return) (cons 'return-not-in-main (cdr body)))
      (else (cons (car body) (mark-return (cdr body)))))))
...
; get the new environment after calling the function
(define get-new-environment
  (lambda (function-name environment function-env)
    (if (null? (get-inaccessible-variable function-name environment))
        (get-global-variable function-name function-env)
        (cons (car (get-inaccessible-variable function-name environment)) (get-global-variable function-name function-env)))))

; change a list of variables to value
(define eval-list
  (lambda (lis environment throw)
    (if (null? lis) '()
        (cons (eval-expression (car lis) environment throw) (eval-list (cdr lis) environment throw)))))

; get the call function environment
(define get-function-environment
  (lambda (global-variable parameter variable)
    (cond
      ((not (eq? (length parameter) (length variable))) (myerror "Mismatched Parameters"))
      (else (cons (cons variable (list parameter)) global-variable)))))

(define get-global-variable
  (lambda (function-name environment)
    (if (exists-in-list? function-name (variables (topframe environment)))
        environment
        (get-global-variable function-name (cdr environment)))))

; get the variables that are not used in the call function
(define get-inaccessible-variable
  (lambda (function-name environment)
    (if (exists-in-list? function-name (variables (topframe environment)))
        '()
        (cons (topframe environment) (get-inaccessible-variable function-name (cdr environment))))))
...
(define get-function-closure
  (lambda (statement)
    (cons (caddr statement) (list (cadddr statement)))))

(define get-function-parameter
  (lambda (statement)
    (cddr statement)))
